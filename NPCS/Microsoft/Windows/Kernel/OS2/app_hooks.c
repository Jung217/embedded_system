    ...
    while (!feof(fp)) {
        ...
        while (ptr != NULL) {
            ...
            if (i == 0) {
                ...
                TaskParameter[j].TaskStartTime = TaskInfo[i];
            }
            else if (i == 2) {
            ...
            TaskParameter[j].TaskReadyTime = 0;
            TaskParameter[j].TaskBlockTime = 0;
            TaskParameter[j].TaskPreemptiveTime = 0;
            i++;
        }
...
void  App_TimeTickHook (void)
{
    ...
    while (ptcb) {
        task_para_set* task_data = (task_para_set*)ptcb->OSTCBExtPtr;
            if (task_data && timeTag >= task_data->TaskStartTime) {
                int gap = timeTag - task_data->TaskStartTime;
                if (task_data->TaskPeriodic > 0 && (gap % task_data->TaskPeriodic) == 0) {
