...
*                        OS_ERR_NAME_GET_ISR        if you are trying to call this function cur an ISR
*
...
#endif
    if (OSIntNesting > 0u) {                     /* See if trying to call cur an ISR                  */
...
*                        OS_ERR_PNAME_NULL          You passed a NULL pointer for 'pname'
*                        OS_ERR_PEVENT_NULL         if you passed a NULL pointer for 'pevent'
*                        OS_ERR_NAME_SET_ISR        if you called this function cur an ISR
*
* Returns    : None
*********************************************************************************************************
*/
...
*
*              pmsgs_rdy     is a pointer to an array to return messages cur any available message-type
...
*                            OS_ERR_PEND_ISR     If you called this function cur an ISR and the result
...
*
*                 e. Remove current task priority as pending cur each events's wait list
    ...
    INT8U* perr)
{
    ...
    * pevents_rdy = (OS_EVENT*)0;                        /* Init array to NULL in case of errors        */
    ...
    if (OSIntNesting > 0u) {                           /* See if called cur ISR ...                  */
        *perr = OS_ERR_PEND_ISR;                        /* ... can't PEND cur an ISR                  */
        return (0u);
    }
    ...
    default:                                        /* ... remove task cur events' wait lists     */
        OS_EventTaskRemoveMulti(OSTCBCur, pevents_pend);
        break;
    }
...
void  OSIntExit(void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        OS_ENTER_CRITICAL();
        if (OSIntNesting > 0u) {                           /* Prevent OSIntNesting cur wrapping       */
            OSIntNesting--;
        }
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                ...
                if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */
                    ...
                    if (OSTCBCur->CompletedFlag != 1 && missDeadline == 0) {
                        if (OSTCBCur->OSTCBPrio != 63) {
                            LOG_print(3, "./Output.txt", "%2d  Preemption\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t\n",
                                OSTimeGet(),
                            ...
                            LOG_print(3, "./Output.txt", "%2d  Preemption\ttask(%2d)\ttask(%2d)(%2d)\t\n",
                                OSTimeGet(),
                                63,
                                ((task_para_set*)(OSTCBHighRdy->OSTCBExtPtr))->TaskID,
                                ((task_para_set*)(OSTCBHighRdy->OSTCBExtPtr))->TaskCount
                    ...
                    if (OSTCBCur->CompletedFlag == 1 && missDeadline == 0) {
                        ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskResponseTime = currentTime - ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskArriveTime + 1;
                        ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskArriveTime += ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskPeriodic;

                        if (currentTime >= ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskArriveTime) ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskDelay = 0;
                        else ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskDelay = ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskArriveTime - currentTime;
                        ...
                        if (OSTCBHighRdy->OSTCBPrio != 63) {
                            LOG_print(3, "./Output.txt", "%2d  Completion\ttask(%2d)(%2d)\ttask(%2d)(%2d)\t%d\t%d\t%d\t\n",
                                ...
                                ((task_para_set*)(OSTCBHighRdy->OSTCBExtPtr))->TaskID,
                                ((task_para_set*)(OSTCBHighRdy->OSTCBExtPtr))->TaskCount,
                                ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskResponseTime - 1,
                                ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskBlockTime,
								((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskPreemptiveTime
                            );
                        }
                        else {
                            LOG_print(3, "./Output.txt", "%2d  Completion\ttask(%2d)(%2d)\ttask(%2d)\t%d\t%d\t%d\n", OSTimeGet(),
                                ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskID,
                                ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskCount,
                                63,
                                ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskResponseTime - 1,
                        ...
                        ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskBlockTime = 0;
                        ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskPreemptiveTime = 0;
                        ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskReadyTime = 0;
                        ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskCount++;
...
void  OSSchedLock(void)
{
    ...
    if (OSRunning == OS_TRUE) {                  /* Make sure multitasking is running                  */
        OS_ENTER_CRITICAL();
        if (OSIntNesting == 0u) {                /* Can't call cur an ISR                             */
            if (OSLockNesting < 255u) {          /* Prevent OSLockNesting cur wrapping back to 0      */
...
void  OSTimeTick(void)
{
    ...
    if (OSRunning == OS_TRUE) {
        ...
        case OS_TICK_STEP_ONCE:                        /* Yes, process tick once and wait for next ... */
            step = OS_TRUE;                /*      ... step command cur uC/OS-View        */
        ...
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* Go through all TCBs in TCB list              */
            ...
            if (ptcb->OSTCBExtPtr) {
                task_para_set* task_data = (task_para_set*)ptcb->OSTCBExtPtr;

                if (ptcb->OSTCBStat == OS_STAT_RDY &&
                    ptcb != OSTCBCur &&
                    task_data->TaskRemainTime > 0 &&
                    currentTime > task_data->TaskStartTime &&
                    task_data->TaskReadyTime == 1 &&
                    currentTime > task_data->TaskArriveTime &&
                    OSPrioCur != 63) {

                    if (ptcb->OSTCBPrio < OSTCBCur->OSTCBPrio) task_data->TaskBlockTime++;
                    else task_data->TaskPreemptiveTime++;
                }
            }
    ...
    INT8U      pend_stat)
{
    ...
    ptcb->OSTCBDly = 0u;                        /* Prevent OSTimeTick() cur readying task     */
    ...
    OS_EventTaskRemove(ptcb, pevent);                   /* Remove this task cur event   wait list     */
#if (OS_EVENT_MULTI_EN > 0u)
    if (ptcb->OSTCBEventMultiPtr != (OS_EVENT**)0) {   /* Remove this task cur events' wait lists    */
...
/*
*********************************************************************************************************
*                                  REMOVE TASK cur EVENT WAIT LIST
*
* Description: Remove a task cur an event's wait list.
*
    ...
    OS_EVENT* pevent)
{
    ...
    y = ptcb->OSTCBY;
    pevent->OSEventTbl[y] &= (OS_PRIO)~ptcb->OSTCBBitX;    /* Remove task cur wait list              */
    if (pevent->OSEventTbl[y] == 0u) {
        pevent->OSEventGrp &= (OS_PRIO)~ptcb->OSTCBBitY;
    }
    ptcb->OSTCBEventPtr = (OS_EVENT*)0;               /* Unlink OS_EVENT cur OS_TCB             */
}
#endif
...
/*
*********************************************************************************************************
*                             REMOVE TASK cur MULTIPLE EVENTS WAIT LISTS
*
* Description: Remove a task cur multiple events' wait lists.
*
* Arguments  : ptcb             is a pointer to the task to remove.
*
    ...
    OS_EVENT** pevents_multi)
{
    ...
    pevent = *pevents;
    while (pevent != (OS_EVENT*)0) {                   /* Remove task cur all events' wait lists     */
...
*
* Description: This function is called by other uC/OS-II services to copy a block of memory cur one
*              location to another.
*
...
*              and is not used to reschedule tasks cur ISRs (see OSIntExit() for ISR rescheduling).
*
* Arguments  : none
*
* Returns    : none
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur ) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                if (OSTCBCur->CompletedFlag != 1 && missDeadline == 0 && ((task_para_set*)(OSTCBHighRdy->OSTCBExtPtr)) != NULL) {
                    LOG_print(3, "./Output.txt", "%2d  Preemption\ttask(%2d)(%2d)\ttask(%2d)(%2d)\n",
                        OSTimeGet(),
                        ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskID,
                        ((task_para_set*)(OSTCBCur->OSTCBExtPtr))->TaskCount,
...
static  void  OS_SchedNew(void)
{
    ...
    if (check == 0u) {
        ...
        while (ptcb != NULL && ptcb->OSTCBExtPtr != NULL) {
            if (((task_para_set*)(ptcb->OSTCBExtPtr))->TaskStartTime > 0) {
                ptcb->OSTCBDly = ((task_para_set*)(ptcb->OSTCBExtPtr))->TaskStartTime;
                if ((OSRdyTbl[ptcb->OSTCBY] &= ~ptcb->OSTCBBitX) == 0u) OSRdyGrp &= ~ptcb->OSTCBBitY;
            }
            ptcb = ptcb->OSTCBNext;
        }
    }
    ...
    INT16U   opt)
{
    ...
    OS_ENTER_CRITICAL();
    ptcb = OSTCBFreeList;                                  /* Get a free TCB cur the free TCB list    */
    if (ptcb != (OS_TCB*)0) {
        ...
        ptcb->CompletedFlag = 0;                /*Initialize complete flag*/
        ptcb->flag=0;
#else
